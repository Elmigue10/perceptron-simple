...
function addRowResponse() {
    for (let i = 0; i < 4; i++) {
        let salida = (((Math.tanh(((weight[0] * trueTable[0][i]) + (weight[1] * trueTable[1][i]))) - ERROR)>=0))?1: -1;
        table.insertRow(-1).innerHTML =
            '<tr><th scope="row" class="text-center">' + (i + 1) + 
            '</th><td class="text-center">' + trueTable[0][i] + 
            '</td><td class="text-center">' + weight[0] + 
            '</td><td class="text-center">' + trueTable[1][i] + 
            '</td><td class="text-center"> ' + weight[1] + 
            '</td><td class="text-center">' + ERROR + 
            '</td><td class="text-center">' + salida + 
            '</td></tr>';
    }
}
